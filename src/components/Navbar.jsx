...
const Navbar = () => {
  ...
  return (
    <nav className={`fixed w-full z-50 transition-all duration-300 transform ${
      ...
      scrolled ? 'glass-effect shadow-sm py-1' : 'bg-transparent py-3'
    }`}>
      <div className="container flex items-center justify-between px-4 md:px-6 lg:px-8">
        {/* Logo - moved closer to center on medium and large devices */}
        <div className="flex-shrink-0 lg:mr-0 md:mr-0">
          <Link to="/" className="flex items-end">
            <h1 className={`font-cormorant text-3xl md:text-4xl font-semibold tracking-wide ${scrolled ? 'text-[#1C4571]' : 'text-[#1C4571]'}`}>
              Feathers
            </h1>
          </Link>
        </div>
        
        {/* Navigation Links - Center - adjusted spacing */}
        ...
        {/* Contact Info - Right - moved closer to center */}
        <div className="hidden lg:flex items-center lg:ml-0 flex-shrink-0">
          <div className="bg-[#1C4571] rounded-full p-1.5 text-white hover:bg-opacity-80 transition-colors">
